...
export const getAllOrders = async (req, res) => {
  try {
    ...
    // For admin, populate phone from user profile if not in order
    if (req.user.isAdmin) {
...
export const createOrder = async (req, res) => {
  try {
    ...
    if (!customer_name || !email || !address || !pincode) {
      return res
        .status(400)
        .json({ error: "Customer name, email, address, and pincode are required" });
    }
